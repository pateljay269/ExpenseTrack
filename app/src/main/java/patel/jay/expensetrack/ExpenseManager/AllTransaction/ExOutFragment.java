 ...
 */
public class ExOutFragment extends Fragment {
    ...
    public ExOutFragment() {
        // Required empty public constructor
    }

    public static void setArrayList(Activity context, int year, String month, boolean order) {
        try {
            String caType = SQLiteHelper.CAT_EX;

            if (!month.equals("") && year != 0) {
                accountArrayList = SqlExTrans.allDetails(context, caType, year, month + " " + year, order, true);
            } else if (month.equals("") && year != 0) {
                accountArrayList = SqlExTrans.allDetails(context, caType, year, order, true);
            } else {
                accountArrayList = SqlExTrans.allDetails(context, caType, order, true);
            }
            setRv(context);
        } catch (Exception e) {
            e.printStackTrace();
...
//            rvCat.setLayoutManager(new LinearLayoutManager(getActivity()));
            ...
            TimeConvert timeConvert = TimeConvert.timeMiliesConvert(System.currentTimeMillis());
            int mm = timeConvert.getMm();
            int yy = timeConvert.getYy();

            setArrayList(getActivity(), yy, MyConstants.monthName(mm), false);
